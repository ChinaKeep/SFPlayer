...
- (instancetype)init{
    if (self = [super init]) {
        ...
        self.keyWindow = [self sf_activeWindow];
...
- (void)setStatusBarHidden:(BOOL)hidden{
    // iOS 13+ no longer exposes statusBarWindow/statusBar via UIApplication.
    // Keep this as a safe no-op to avoid runtime crashes on modern systems.
    (void)hidden;
}
